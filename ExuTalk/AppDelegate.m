...
@interface AppDelegate()<XMPPReconnectDelegate>
{
...
-(void)showConvScreen{
    ...
    //
    //    if (!self.convVc) {
    //        self.convVc    =   [ConversationVC instantiateFromStoryboard];
    //    }
    //
    //    if (!self.navigationController) {
    //        self.navigationController =   [[UINavigationController alloc] initWithRootViewController:self.convVc];
...
//}
//
//- (void)setupLocalNotifications:(XMPPUserCoreDataStorageObject*)userMessage {
...
- (void)application:(UIApplication *)application didReceiveLocalNotification:(UILocalNotification *)notification {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kGOTMESSAGENOTIFICATION object:self];
...
{
	return [xmppCoreDataStorage mainThreadManagedObjectContext];
}
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    //    [self setupLocalNotifications:nil];
}
...
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    //    NSXMLElement *vcard =   [iq childElement];
    //    NSArray *att    =   [vcard namespaces];
    //    att   =   [att[0] componentsSeparatedByString:@"="];
    //    NSString *str =   att[1];
    //
    //    if ([str isEqualToString:@"vcard-temp"]) {
    //        [xmppvCardTempModule vCardTempForJID:[iq from] shouldFetch:NO];
    //        [[NSNotificationCenter defaultCenter] postNotificationName:kNotifyVardRecived
    //                                                            object:nil
    //                                                          userInfo:nil];
    //    }
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    //
    //    [self archiveMessageViaDelegate:message
    //                           outgoing:NO];
    ...
    if ([message isChatMessageWithBody]){
        ...
        NSArray *listOfSender   =   [from componentsSeparatedByString:@"/"];
        listOfSender = [listOfSender[1] componentsSeparatedByString:@" "];
        m[@"sender"]    =   listOfSender[0];
        
        XMPPUserCoreDataStorageObject *user = [xmppRosterStorage userForJID:[message from]
		                                                         xmppStream:xmppStream
		                                               managedObjectContext:[self managedObjectContext_roster]];
        m[@"senderEmail"]   =   user.jidStr;
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kNewMessageRecieved
                                                            object:nil
                                                          userInfo:@{@"newMessageRecievedForJid":user.jidStr}];
    }
}
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    if([xmppvCardStorage shouldFetchvCardTempForJID:[presence from] xmppStream:xmppStream]){
        [xmppvCardTempModule fetchvCardTempForJID:[presence from]];
    }
}
